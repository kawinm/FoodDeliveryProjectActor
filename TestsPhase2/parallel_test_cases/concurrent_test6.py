...
# Scenario:
#  Check if an agent gets assigned to another order soon after he 
#  delivered an order.
...
def test():
    ...
    # Get status and agentId field of order with id 1001
    http_response = requests.get("http://localhost:8081/order/1001")
    status = http_response.json().get("status")
    agentAssigned = http_response.json().get("agentId")
    if(status != "assigned" or agentAssigned != 201):
        return Fail
    return Pass
